...
if (obj_player.state == 7 || obj_player.state == 25 || obj_player.state == 20) {
	depth = -1;
...
with (obj_player) {
	if (!(state == 25 || state == 7 || state == 29 || state == 20 || state == 21 || state == 26 || state == 27 || state == 28 || state == 30)) {
		other.x = x;
		other.y = y;
		other.state = 83;
		other.image_index = 0;
...
if (place_meeting(x, y, obj_player)) {
	...
	if (obj_player.state == 20 && floor(obj_player.image_index) == 2 && obj_player.throwforce < 20) {
		x = obj_player.x;
		y = obj_player.y;
		state = 82;
		hsp = -image_xscale * 7;
	...
	if (obj_player.state == 29 && floor(obj_player.image_index) == 2 && obj_player.throwforce < 20) {
		x = obj_player.x;
		y = obj_player.y;
		hsp = -image_xscale * 5;
		state = 82;
	...
	if (obj_player.state == 21 && obj_player.slamming == 0) {
		depth = -7;
		x = obj_player.x;
		y = obj_player.y + 20;
	}
	
	if (obj_player.state == 21 && obj_player.slamming == 1) {
		x = obj_player.x;
		y = obj_player.y;
		state = 82;
		hsp = -image_xscale * 10;
